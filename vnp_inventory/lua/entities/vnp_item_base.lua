...
if CLIENT then
    ...
    function ENT:Draw()
        ...
        if self:forSale() then
            ...
            self.iteminfo.salePrice:SetSize(w*1.1,h*1.5)
            self.iteminfo.salePrice:Center()
            ...
            self.iteminfo.salePrice.Paint = function(s,w,h)
                ...
                draw.RoundedBox(0,0,0,w,h,Color(50,50,54))
                local xW, xH = VNP:DrawText("FOR SALE  ", 22, w*.05,h/6, Color(0,150,0), 0)
                VNP:DrawText("$"..VNP:FormatNumber(self:getPrice()), 22, w*.05+xW,h/6, Color(0,200,0), 0)
            end
