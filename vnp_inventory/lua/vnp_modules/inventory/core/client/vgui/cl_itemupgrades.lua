...
function PANEL:Init()
    local w,h = VNP:resFormatW(275), VNP:resFormatH(275)
    ...
    self.Item:Receiver("Swap Inventory Item", function(s,p,b)
        ...
        if p[1].Data.Type == "WEAPON" or self.DoSockets then
            ...
            for socket,gem in ipairs(p[1].Data.Sockets) do
                ...
                if gem then
                    ...
                    self.Sockets.Items[socket].DoRightClick = function(s)
                        ...
                        s.options = vgui.Create("VNPOptionMenu")
            ...
            self.SocketGem = vgui.Create("VNPButton", self)
        ...
        elseif p[1].Data.Type == "SCROLL" then
            ...
            self.UpgradeScroll = vgui.Create("VNPButton", self)
        ...
        elseif p[1].Data.Type == "SUIT" then
            ...
            self.RepairSuit = vgui.Create("VNPButton", self)
            ...
            local text = "Repair Suit - $"..VNP:FormatNumber(cost)
...
function PANEL:PostPaint(w,h)
    ...
    local name = self._Data.Label or self._Data.Name
    ...
    tW,tH = VNP:DrawText(name, 26, x+w/2, y, color_white, 1)
    y=y+tH
    
    if self._Data.UID then
        VNP:DrawText(uid, 30, x+w*.1, y, color_white, 0)
    else
        local color = HSVToColor(RealTime() * 20 % 360, 1, 1)
        VNP:DrawText(uid, 30, x+w*.1, y, color, 0)
    end

    tW,tH = VNP:DrawText(rarity, 26, x+w/2, y*1.1, rColor, 1)
    y=y+tH+spacer*0.5
    ...
    for stat,val in SortedPairsByValue(self.Stats, true) do
        ...
        tW,tH = VNP:DrawText(label, 22, x+w*.1, y+spacer*1.5, color_white, 0)
        VNP:DrawText(prefix..val..suffix, 26, x+w*.9, y+spacer*1.5, Color(255,50,50), 2)
        ...
        if self._Data.Type == "WEAPON" && self.iData.Modifiers[stat] then
            ...
            VNP:DrawText(v, 26, x+w*.6, y+spacer*1.5, color_white, 2)
        end
        y=y+tH*1.1
    end
end
