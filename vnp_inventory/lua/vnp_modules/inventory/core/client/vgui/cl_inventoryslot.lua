...
function PANEL:Paint(w,h)
    ...
    color = self.Color && VNP:Darken(color, 25) or Color(0, 0, 0, 100)
    ...
    if self.Data && self.Data.Type == "SUIT" then
        ...
        local suitMaxHealth = (self.Data["SuitHealth"] ~= nil and self.Data["SuitHealth"] ~= 0) and VNP.Inventory:CalculateMaxStat(self.Data, "SuitHealth") or 0
        local suitMaxArmor = (self.Data["SuitArmor"] ~= nil and self.Data["SuitArmor"] ~= 0) and VNP.Inventory:CalculateMaxStat(self.Data, "SuitArmor") or 0

        local suitHealth = math.Round((self.Data["SuitHealth"] ~= nil and self.Data["SuitHealth"] ~= 0) and self.Data["SuitHealth"] or 0)
        local suitArmor = math.Round((self.Data["SuitArmor"] ~= nil and self.Data["SuitArmor"] ~= 0) and self.Data["SuitArmor"] or 0)
        ...
        surface.SetDrawColor(Color(10,10,10,100))
        surface.DrawRect(c*2, h-c*5, width, c*1.5)

        surface.SetDrawColor(Color(1,156,254))
        surface.DrawRect(c*2, h-c*5, math.min(suitArmor*(width/suitMaxArmor), width), c*1.5)
    end
...
function PANEL:Think()
    ...
    if self.Data and (!self.Model or !IsValid(self.Model)) then
       ...
       self.Model = VNP.Inventory:CreateModel(itemData.Class, 0,0, self:GetWide(), self:GetTall(), self, itemData.Model, IsColor(color) && color or false, niggers) 
    end
end
