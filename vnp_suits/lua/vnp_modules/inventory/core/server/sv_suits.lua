...
function PLAYER:RegenSuit()
    ...
    local suitMaxArmor = VNP.Inventory:CalculateMaxStat(data, "SuitArmor")
...
function VNP.Inventory:InitializeSuits()
    hook.Add("EntityTakeDamage", "VNP.SuitDamage", function(ply, dmginfo)
        ...
        if IsValid(wep) then
            if Resistances[wep:GetClass()] then
                resist = resist - (Resistances[wep:GetClass()] / 100)
            end

            if table.HasValue(VNP.Inventory.EnergyWeapons, wep:GetClass()) and item.Rarity.Modifiers and item.Rarity.Modifiers["SuitEnergyResist"] then
                resist = resist - (item.Rarity.Modifiers["SuitEnergyResist"] / 100)
            end
        end

        if item["SuitResist"] or (item.Rarity.Modifiers and item.Rarity.Modifiers["SuitResist"]) then
    ...
    hook.Add("PlayerSay", "VNP.Suits.PlayerSay", function(ply, msg)
        ...
        if msg == "/dropsuit" then
            if ply:GetNWBool("eventmode", false) == true then ply:ChatPrint("{*Server Is In Event Mode You Cannot Drop A Suit Currently!}") return end
            ...
            ply:NNotify("Generic", "Dropping suit in 5 seconds!")
